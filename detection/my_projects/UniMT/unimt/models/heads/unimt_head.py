...
class UniMTHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(UniMTHead, self).__init__(init_cfg=init_cfg)
    ...
    def init_weights(self):
        super(UniMTHead, self).init_weights()
    ...
    def _rv_pe(self, img_feats, img_metas):
        ...
        coords_h, coords_w, coords_d = torch.meshgrid([coords_h, coords_w, coords_d], indexing='ij')
    ...
    def forward(self, pts_feats, img_feats, img_metas):
        ...
        img_feats = [img_feat + self._rv_pe(img_feat, img_metas).permute(0, 3, 1, 2) for img_feat in img_feats]
        img_feat_flatten = torch.cat([img_feat.flatten(2).permute(0, 2, 1) for img_feat in img_feats], dim=1).contiguous()
        img_feat_shape = torch.tensor([img_feat.shape[-2:] for img_feat in img_feats], device=img_feat_flatten.device)
        
        pts_pos = self.bev_embedding(pos2embed(self.coords_bev.to(pts_feat.device), num_pos_feats=self.hidden_dim))
        pts_feat = pts_feat + pts_pos.permute(2, 0, 1).unsqueeze(0).repeat(pts_feat.shape[0], 1, 1, 1)
        pts_feat_flatten = pts_feat.flatten(2).permute(0, 2, 1).contiguous()
        pts_feat_shape = torch.tensor([pts_feat.shape[-2:]], device=pts_feat_flatten.device)
        ...
        outs_dec = self.transformer_decoder(query, query_pos, pts_feat_flatten, pts_feat_shape, img_feat_flatten, img_feat_shape,
