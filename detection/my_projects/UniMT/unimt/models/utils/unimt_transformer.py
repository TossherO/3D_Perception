...
class UniMTTransformerDecoder(TransformerLayerSequence):
    ...
    def __init__(self,
        ...
        super(UniMTTransformerDecoder, self).__init__(*args, **kwargs)
    ...
    def forward(self, query, query_pos, pts_feat, pts_feat_shape, img_feat, img_feat_shape, 
                reference_points, pc_range, img_metas, attn_masks=None, layer_idx=None):
        """Forward function for `TransformerDecoder`.
        Args:
            query, query_pos (Tensor): [bs, num_query, embed_dims]
            pts_feat (Tensor): [bs, pts_l * pts_w, embed_dims]
            pts_feat_shape (Tensor): [1, 2], [(pts_l, pts_w)]
            img_feat (Tensor): [bs * num_cam, sum(img_h_i * img_w_i for i in num_level), embed_dims]
            img_feat_shape (Tensor): [num_level, 2], [(img_h_0, img_w_0), (img_h_1, img_w_1), ...]
        ...
        if layer_idx is None:
            ...
            for layer in self.layers:
                query = layer(query, query_pos, pts_feat, pts_feat_shape, img_feat, img_feat_shape, 
            ...
            query = self.layers[layer_idx](query, query_pos, pts_feat, pts_feat_shape, img_feat, img_feat_shape,
                                            reference_points, pc_range, img_metas, attn_masks)
            if self.post_norm is not None:
...
@MODELS.register_module()
class UniMTTransformerDecoderLayer(BaseModule):
    ...
    def _forward(self, query, query_pos, pts_feat, pts_feat_shape, img_feat, img_feat_shape, 
                    reference_points, pc_range, img_metas, attn_masks):
        ...
        for layer in self.operation_order:
            if layer == 'self_attn':
                ...
                query = self.attentions[attn_index](
                    query, query_pos, pts_feat, pts_feat_shape, img_feat, img_feat_shape, 
    ...
    def forward(self, query, query_pos, pts_feat, pts_feat_shape, img_feat, img_feat_shape, 
        ...
        if self.use_checkpoint and self.training:
            x = cp.checkpoint(self._forward, query, query_pos, pts_feat, pts_feat_shape, img_feat, img_feat_shape, 
                                reference_points, pc_range, img_metas, attn_masks)
        else:
            x = self._forward(query, query_pos, pts_feat, pts_feat_shape, img_feat, img_feat_shape, 
                                reference_points, pc_range, img_metas, attn_masks)
        return x
...
class DeformableAttention2MultiModality(BaseModule):
    ...
    def __init__(self, embed_dims=256, num_heads=8, num_level=3, num_points=13, dropout=0.1, im2col_step=64, batch_first=True):
        ...
        self.img_attention_weights = nn.Linear(embed_dims, num_heads * num_level * num_points)
    ...
    def forward(self, query, query_pos, pts_feat, pts_feat_shape, img_feat, img_feat_shape, 
        ...
        pts_len, img_len = pts_feat.shape[1], img_feat.shape[1]
        ...
        pts_feat = self.pts_proj(pts_feat).view(bs, pts_len, self.num_heads, -1)
        img_feat = self.img_proj(img_feat).view(bs * num_cam, img_len, self.num_heads, -1)

        # get sampling offsets and attention
        identity = query
        query = query + query_pos
        sampling_offsets = self.sampling_offsets(query).view(bs, num_query, self.num_points, 3)  # [bs, num_query, num_points, 3]
        pts_attention_weights = self.pts_attention_weights(query).view(
            bs, num_query, self.num_heads, self.num_points).softmax(dim=-1).squeeze(3)  # [bs, num_query, num_heads, 1, num_points]
        ...
        img_attention_weights = self.img_attention_weights(query_cam).view(
            bs * num_cam, num_query, self.num_heads, self.num_level * self.num_points).softmax(dim=-1).view(
                bs * num_cam, num_query, self.num_heads, self.num_level, self.num_points)  # [bs * num_cam, num_query, num_heads, num_level, num_points]

        # get pts sampling points
        reference_points = reference_points * (pc_range[3:6] - pc_range[0:3]) + pc_range[0:3]
        sampling_points = reference_points.unsqueeze(2) + sampling_offsets  # [bs, num_query, num_points, 3]
        ...
        pts_points = pts_points[:, :, None, None, :, :].repeat(1, 1, self.num_heads, 1, 1, 1)  # [bs, num_query, num_heads, 1, num_points, 2]
        
        # get img sampling points
        img_points = torch.matmul(lidars2imgs[:, :, None, None, :, :], sampling_points[:, None, :, :, :, None]).squeeze(-1)  # [bs, num_cam, num_query, num_points, 4]
        ...
        img_points = img_points.view(bs*num_cam, num_query, 1, 1, self.num_points, 2).repeat(
            1, 1, self.num_heads, self.num_level, 1, 1)  # [bs * num_cam, num_query, num_heads, num_level, num_points, 2]
        
        # get pts, img features
        out_pts = MultiScaleDeformableAttnFunction.apply(
            pts_feat, pts_feat_shape, torch.tensor([0]).to(query.device), pts_points, pts_attention_weights, self.im2col_step)
        level_start_index = torch.cat((img_feat_shape.new_zeros((1, )), img_feat_shape.prod(1).cumsum(0)[:-1]))
        out_img = MultiScaleDeformableAttnFunction.apply(
            img_feat, img_feat_shape, level_start_index, img_points, img_attention_weights, self.im2col_step)
        
        # get output
        out_img = out_img.view(bs, num_cam, num_query, embed_dims).sum(1)
