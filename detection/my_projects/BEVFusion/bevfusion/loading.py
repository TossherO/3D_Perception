...
@TRANSFORMS.register_module()
class LoadMultiViewImageFromFilesCoda(LoadMultiViewImageFromFiles):
    """Load multi channel images from a list of separate channel files.
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        Returns:
            ...
            Added keys and values are described below.
                ...
                - img_norm_cfg (dict): Normalization configuration of images.
        """
        filename, cam2img, lidar2cam, lidar2img, cam2lidar = [], [], [], [], []
        for _, cam_item in results['images'].items():
            filename.append(cam_item['img_path'])
            lidar2cam.append(cam_item['lidar2cam'])
            ...
            cam2img.append(cam2img_array)
            lidar2img.append(cam2img_array @ lidar2cam_array)
            cam2lidar_array = np.eye(4).astype(np.float64)
            cam2lidar_array[:3, :3] = lidar2cam_array[:3, :3].T
            cam2lidar_array[:3, 3] = -lidar2cam_array[:3, :3].T @ lidar2cam_array[:3, 3]
